...
import fileSystemAnalyzer.AnalyzerStorage;
...
public final class DBLayer implements AnalyzerStorage {
    ...
    public List<String> selectPathsOf(String value) {
        ...
        String getPathSQL = "SELECT path FROM " + this.mainTableName
                + " WHERE value=?";
    ...
    public Map<String, List<String>> selectGroupedValues(Integer minCount) {
        ...
        String hashedValuesSQL = "SELECT value, group_concat(path, '|') count FROM "
                + this.mainTableName + " GROUP BY value " + having;
    ...
    private static DBLayer getNew(Class<? extends DBSingleStorage> plugin) {
        ...
        dbPath.append(System.getProperty("user.dir"))
        ...
        if (Files.exists(databasePath.toPath())) {
            ...
            if (!conn.isOpen()) {
                try {
                    ...
                    _db = new DBLayer(PluginId);
                    _db.Connection = conn;
                    String CREATE_MAIN_TABLE = "CREATE TABLE IF NOT EXISTS __table__ ( parent NVARCHAR(2048) NULL, path NVARCHAR(2048) NULL, value TEXT NULL)"
                            .replace("__table__", _db.mainTableName);
                    ...
                    new DBLayerException(
                            DBLayerException.UNABLE_TO_CREATE_DBLAYER_INSTANCE, e)
                            .printStackTrace();
                }
            }
        }
    ...
    public static String selectInstanceValueOf(DBLayer dBInstance, String path,
       ...
       // System.out.println("dBInstance hascode " +dBInstance.hashCode());
        ...
        String getValueSQL = "SELECT " + dBFieldName + " FROM "
                + dBInstance.mainTableName + " WHERE path=?";
    ...
    public static List<String> selectValuesOf(DBLayer dBInstance,
        ...
        String getValueSQL = "SELECT " + dBFieldName + " FROM "
                + dBInstance.mainTableName + " WHERE path in (";
